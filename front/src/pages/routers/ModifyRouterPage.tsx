...
export default function ModifyRouterPage() {
    ...
    const [router, setRouter] = useState<RouterDao>({routerName: "", routerDetails: "", routerSyntax: "", routerIp: "", routerUsername: "", routerPassword: "", communicationDeviceRouter: false, routerPhone: "", id: 0});
    ...
    function convertFromBoolToStringRouterType(e) {
        if(e === true)
          return "Router dispozitive";
        else
          return "Router notificari";
    }
        ...
        <div className="grid-cols-1 mb-12 grid gap-12 px-4 lg:grid-cols-2 xl:grid-cols-1 justify-items-center">
            <form className="mt-8 mb-2 w-80 max-w-screen-lg sm:w-96 lg:p-0 p-7">
                    <div className="mb-4 flex flex-col gap-6"> 
                        ...
                        <Input value={router.routerName} required size="lg" label="Nume" onChange={(e) => { setRouter({...router, routerName: e.target.value})}}/>
                        <Input value={router.routerDetails} required size="lg" label="Detalii" onChange={(e) => { setRouter({...router, routerDetails: e.target.value})}}/>
                        <Input value={router.routerSyntax} required size="lg" label="Sintaxa" onChange={(e) => { setRouter({...router, routerSyntax: e.target.value})}}/>
                        <Input value={router.routerIp} required size="lg" label="Ip" onChange={(e) => { setRouter({...router, routerIp: e.target.value})}}/>
                        <Input value={router.routerUsername} required size="lg" label="Utilizator" onChange={(e) => {setRouter({...router, routerUsername: e.target.value})}}/>
                        <Input required type="password" size="lg" label="Parola" onChange={(e) => {setRouter({...router, routerPassword: e.target.value})}}/>
                        <Input required type="password" size="lg" label="Repeta parola" onChange={(e) => {setRepeatPassword(e.target.value)}} />
                        ...
                        <GenericDropdown array={["Router dispozitive", "Router notificari"]} defaultValue={convertFromBoolToStringRouterType(router.communicationDeviceRouter)} onChange={(e) => {onChangeRouterType(e)}} />
                        
                    </div>
                    {/* <Button onClick={change} className="mt-6" fullWidth>Modifica</Button> */}

                    <div className="mb-4 flex flex-col gap-6"> 
                      <button onClick={change} className="buttonRouterAdd">
                      Modifica
                      </button>
                    </div>
