      ...
      <div className="mt-12 mb-8 flex flex-col gap-12">
        <Card>
          ...
          <div className="flex w-max gap-4 mt-1 ml-5">
            <Input required size="lg" label="Cuvant cautare" type="text" onChange={(e) => { setAuditRequest({...auditRequest, entityName: e.target.value})}}/>
            <Input required size="lg" label="Data inceput" type="datetime-local" onChange={(e) => { setAuditRequest({...auditRequest, start: e.target.value})}}/>
            <Input required size="lg" label="Data final" type="datetime-local" onChange={(e) => { setAuditRequest({...auditRequest, stop: e.target.value})}}/>
            {/* <Button onClick={reloadState} fullWidth variant="gradient" className="flex items-center gap-3 bg-gradient-to-r from-indigo-300 to-blue-500 bg-cover"><MagnifyingGlassIcon strokeWidth = {2} className = "h-5 w-5"/> Cauta </Button> */}
              <div>
          ...
          <Pagination _page={pagination} totalCount={audit?.totalElements} onPageChanged={onPageChange}/>
            <table className="w-full min-w-[640px] table-auto">
              <thead>
                <tr>
                  {["Acces", "Data", "Entitate", "Eveniment", "Ip", "Utilizator", "", ""].map((el) => (
                    <th
                      key={el}
