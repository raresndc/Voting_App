...
export default function AddDevicesPage() {
    ...
    async function createDevice(ev) {
      ...
      try {
          ...
          Swal.fire({title: 'Loading ... ', text: 'Asteptati inrolarea dispozitivului.', allowEscapeKey: false, allowOutsideClick: false, didOpen: () => { Swal.showLoading(); }})

          await saveDevice(device);

          Swal.fire({icon: 'success', title: 'Succes!', text: 'Dispozitivul-ul a fost adaugat cu succes!'})
        ...
        <div className="grid-cols-1 mb-12 grid gap-12 px-4 lg:grid-cols-2 xl:grid-cols-1 justify-items-center">
            <form className="mt-8 mb-2 w-80 max-w-screen-lg sm:w-96 lg:p-0 p-7">
                    <div className="mb-4 flex flex-col gap-6"> 
                        ...
                        <Input maxLength={10} value={device.deviceMsisdn} required size="lg" label="Telefon" onChange={(e) => { setDevice({...device, deviceMsisdn: e.target.value})}}/>     

                        <GenericDropdown array={deviceTypes.content.map(deviceType => deviceType.deviceTypeName)} defaultValue={"Selecteaza tip dispozitiv"} onChange={(e) => {setDevice({...device, elDeviceType: findDeviceType(e)})}} />                     
                        
                    </div>

                    {/* <Button onClick={createDevice} className="mt-6" fullWidth>Inrolare</Button> */}
