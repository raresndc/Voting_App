...
export default function ModifyDeviceTypePage() {
    ...
    async function change(ev) {
      ...
      try {
        ...
        Swal.fire({icon: 'success', title: 'Succes!', text: 'Tip device a fost actualizat cu succes!'});
      } catch(err) {
        ...
        <div className="grid-cols-1 mb-12 grid gap-12 px-4 lg:grid-cols-2 xl:grid-cols-1 justify-items-center">
            <form className="mt-8 mb-2 w-80 max-w-screen-lg sm:w-96 lg:p-0 p-7">
                    <div className="mb-4 flex flex-col gap-6"> 
                        ...
                        <Input value={deviceType?.deviceTypeDetails} required size="lg" label="param2" onChange={(e) => setDeviceType({...deviceType, deviceTypeDetails: e.target.value})} />
                        
                    </div>
                    {/* <Button onClick={change} className="mt-6" fullWidth>Modifica</Button> */}

                    <div className="mb-4 flex flex-col gap-6"> 
                      <button onClick={change} className="buttonAddUser">
                      Modifica
                      </button>
                    </div>
            </form>
