...
public class CustomUserDetailsService implements UserDetailsService {
	...
	public List<User> getAllAdmins() {
		...
		for(User user: allUsers) {
			if(user.getRoles().stream().filter(roles -> roles.getName().equals(UsersRoles.SUPER_USER) ||
					roles.getName().equals(UsersRoles.ADMIN_USERS) || 
					roles.getName().equals(UsersRoles.SISTEM_ADMIN)
					).count() > 0 ) {
